...
When the user first interacts with Codex in this repository (e.g., "Hello", "New project", "Let's go"), conduct this interview.
...
- This repository includes selectable Codex CLI profiles in `.codex/config.toml`. See `docs/codex-workflow.md`.

## Working with local game files (Codex CLI – mandatory workflow)

**Note:** All `Source` paths in this document are **examples**. Your game may store the same artifacts in different locations (Steam/Epic/GOG variants, mod managers, portable installs, or custom folder layouts). If an artifact is not found at the example path, locate the corresponding file/folder anywhere under your game install and copy it to the specified `workspace-input` destination.

Codex **cannot be configured to automatically browse arbitrary directories on your system** (such as a game install folder) without your explicit involvement. This is by design. Even with Codex CLI, filesystem access is limited by the sandbox and by your approvals.

Therefore, this template uses a **copy-into-workspace workflow**. When Codex needs to inspect game data, you must copy specific files from the game installation into this repository.

### Why this is required
- Keeps Codex runs reproducible and reviewable.
- Prevents accidental access to unrelated personal data.
- Ensures that all analyzed artifacts are versionable and auditable.

### Canonical workspace location
All copied artifacts MUST go under:

```
Accessibility-Mod-Template/workspace-input/
```

Create subfolders as instructed below. Do not copy files elsewhere.

---

### 1. Logs (always copy first)

**Source (example):**
```
<GAME_INSTALL_DIR>/MelonLoader/Latest.log
<GAME_INSTALL_DIR>/MelonLoader/Logs/
```

**Destination:**
```
Accessibility-Mod-Template/workspace-input/logs/
```

**Copy:**
- `Latest.log`
- The full `Logs/` directory if it exists

Purpose:
- Verify loader version
- Identify runtime errors
- Detect Unity version and assemblies loaded

---

### 2. Managed assemblies (only when code inspection is needed)

**Source:**
```
<GAME_INSTALL_DIR>/<GAME_NAME>_Data/Managed/
```

**Destination:**
```
Accessibility-Mod-Template/workspace-input/managed/
```

**Copy:**
- Entire `Managed/` directory OR
- At minimum:
  - `Assembly-CSharp.dll`
  - `UnityEngine*.dll` (if referenced)

Do NOT decompile inside the game directory.

---

### 3. Decompiled output (if you already decompiled)

**Source (tool-dependent):**
```
<IDa / dnSpy / ILSpy output directory>
```

**Destination:**
```
Accessibility-Mod-Template/workspace-input/decompiled/
```

Rules:
- Keep original directory structure
- Do not rename symbols post-export

---

### 4. UI hierarchy / screenshots (UI accessibility work)
...
**Destination:**
```
Accessibility-Mod-Template/workspace-input/ui/
```

Include:
- Tool name used to extract UI
- Game version (text file in same folder)

---

### 5. How to tell Codex what you copied

After copying files, explicitly state:
- What you copied
- From which path
- Why it was copied
...
```
I copied:
- Latest.log from <GAME_INSTALL_DIR>/MelonLoader/
- Assembly-CSharp.dll from <GAME_INSTALL_DIR>/<GAME_NAME>_Data/Managed/

They are now under workspace-input/logs and workspace-input/managed.
```

Codex MUST NOT assume additional files exist unless explicitly stated.

---

### What Codex must NEVER do
- Browse outside the repository
- Assume local paths not copied into workspace
- Ask to scan the entire disk

If additional files are needed, Codex must request them explicitly with a justification.

---

## Variant: Unity IL2CPP + MelonLoader (Unity 2021+ / Unity 6)

Some Unity games (including Unity 6 titles) do NOT use the classic
`<GAME_NAME>_Data/Managed` layout. When using **MelonLoader with IL2CPP**,
assemblies and loader binaries may be located under the MelonLoader directory.

### Loader runtime (net6)

**Example source path:**
```
<GAME_INSTALL_DIR>/MelonLoader/net6/
```
...
**Destination:**
```
Accessibility-Mod-Template/workspace-input/loader/net6/
```

Purpose:
- Identify MelonLoader version and runtime behavior
- Verify Harmony patching environment

### IL2CPP assemblies

**Example source path:**
```
<GAME_INSTALL_DIR>/MelonLoader/Il2CppAssemblies/
```
...
- `UnityEngine.UI.dll` OR `UnityEngine.UIModule.dll` (copy what exists)

**Destination:**
```
Accessibility-Mod-Template/workspace-input/managed/il2cpp/
```

Notes:
- These are IL2CPP-related artifacts, not classic Mono-managed assemblies.
- Analysis capabilities differ from standard `Managed/` layouts.

---

## What analysis is possible / not possible (IL2CPP)

**Possible:** runtime observation, UI structure analysis, focus/navigation modeling, log-based diagnostics, and hook-based accessibility layers.

**Not possible:** reading method bodies, source-level UI logic analysis, or direct inspection of event-handler implementations.

See also:
- `docs/il2cpp-accessibility-strategy.md`

---

## Codex setup pointers

For Codex CLI installation and the step-by-step project startup workflow, see:
